...
def addProblem():
    ...
    if form.validate_on_submit():
        problem = Problem(\
            ...
            urlTitle=''.join((form.title.data).split()).lower(),\
            body=form.body.data,\
            timeLimit = form.timeLimit.data,\
            memoryLimit = form.memoryLimit.data\
        )
        problemTestCases = ProblemTestCases(\
            problem=problem,\
            input = form.testCaseInput.data,\
            output = form.testCaseOutput.data,\
        )
        db.session.add(problem)
        db.session.add(problemTestCases)
        db.session.commit()
        flash('Problem Succesfully Added!')
